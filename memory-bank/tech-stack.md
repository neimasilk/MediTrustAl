...
Based on the project proposal (meditrustal_proposal_en_detailed_v2), the following tech stack is recommended:
...
* **Blockchain Platform:**
  ...
  * **Smart Contracts:**
    * **MVP:** Solidity with Hardhat development environment (version ^2.24.1 from `package.json`)
...
* **Off-Chain Storage:**
  ...
  * **Deployment Location:**
    ...
    * **Production:** Deployed on hospital servers or PIPL-compliant secure cloud infrastructure, as specified in product-design-document.md which mentions "Raw medical data stored off-chain (hospital servers/secure cloud)"
...
* **Backend Development:**
  ...
  * **Rationale for FastAPI (MVP):**
    ...
    * Key dependencies and their versions for the MVP are:
        ...
        - uvicorn[standard] (from `requirements.txt`)
        - pydantic[email]>=2.0.0 (from `requirements.txt`)
        - python-jose[cryptography]>=3.3.0 (from `requirements.txt`)
        - passlib[bcrypt]>=1.7.4 (from `requirements.txt`)
        - psycopg2-binary>=2.9.0 (from `requirements.txt`)
        - alembic>=1.12.0 (from `requirements.txt`)
        - sqlalchemy>=2.0.0 (from `requirements.txt`)
        - web3>=6.0.0 (from `requirements.txt`)
        - python-dotenv>=0.19.0 (from `requirements.txt`)
        - base58>=2.1.0 (from `requirements.txt`)
        - black (from `requirements.txt`)
        - flake8 (from `requirements.txt`)
        - pytest>=7.0.0 (from `requirements.txt`)
        - pytest-asyncio>=0.21.0 (from `requirements.txt`)
        - httpx>=0.24.0 (from `requirements.txt`)
        - python-multipart>=0.0.6 (from `requirements.txt`)
        - pydantic-settings (from `requirements.txt`)
        - respx (from `requirements.txt`)
        - pytest-mock (from `requirements.txt`)
...
* **Frontend Development (Patient, Doctor, Researcher, Admin Portals):**
  * **Framework:** **React.js** (version ^19.1.0 from `frontend/package.json`)
  * **Rationale:**
